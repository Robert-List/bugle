                               ...
                               const regmatch_t *pmatch)
{
    ...
    {
        ans = search_replace(ans, "$W", "(writer)");
    }
...
static void write_type_dumpers(FILE *f)
{
    ...
    for (list<Type>::iterator i = types.begin(); i != types.end(); i++)
    {
        ...
        fprintf(f,
                "static void _budgie_dump_%s(%s, int count, bugle_io_writer *writer)\n"
                "{\n",
        ...
        if (i->overrides.count(OVERRIDE_DUMP))
            custom_code +=
                "    if (" + i->overrides[OVERRIDE_DUMP] + ") return;\n";
        ...
        if (bitfield_map.count(i->node))
        {
            ...
            fprintf(f,
                    ...
                    "    };\n"
                    "%s"
                    "    _budgie_dump_bitfield(*value, writer, tokens, %d);\n"
                    "}\n"
                    "\n",
        ...
        case ENUMERAL_TYPE:
            ...
            while (tmp != NULL_TREE)
            {
                ...
                {
                    fprintf(f,
                            "    case %s: bugle_io_puts(\"%s\", writer); break;\n",
            ...
            fprintf(f,
                    "    default: bugle_io_printf(writer, \"%%ld\", (long) *value);\n"
                    "    }\n");
            break;
        case INTEGER_TYPE:
            fprintf(f,
        ...
        case REAL_TYPE:
            ...
            fprintf(f,
                    "%s"
                    "    bugle_io_printf(writer, \"%%Lg\", (long double) *value);\n",
                    custom_code.c_str());
#else
            fprintf(f,
                    "%s"
                    "    bugle_io_printf(writer, \"%%g\", (double) *value);\n",
                    custom_code.c_str());
#endif
            break;
        case ARRAY_TYPE:
            ...
            if (type_map.count(child))
            {
                ...
                fprintf(f,
                        "        budgie_dump_any_type(%s, &(*value)[i], -1, writer);\n",
                        define.c_str());
            }
            else
                fprintf(f,
                        "        bugle_io_puts(\"<unknown>\", writer);\n");
            fprintf(f,
                    "        if (i < asize - 1)\n"
                    "            bugle_io_puts(\", \", writer);\n"
                    "    }\n"
                    "    if (asize < 0)\n"
                    "        bugle_io_puts(\"<unknown size array>\", writer);\n"
                    "    bugle_io_puts(\" }\", writer);\n");
            break;
        case POINTER_TYPE:
            ...
            fprintf(f,
                    ...
                    "    else bugle_io_printf(writer, \"%%p\", (void *) *value);\n",
                    custom_code.c_str());
            if (type_map.count(child))
            {
                ...
                fprintf(f,
                        ...
                        "        if (count < 0)\n"
                        "            budgie_dump_any_type(%s, *value, -1, writer);\n"
                        "        else\n"
                        "        {\n"
                        // pointer to array
                        "            bugle_io_puts(\"{ \", writer);\n"
                        ...
                        "            {\n"
                        "                budgie_dump_any_type(%s, &(*value)[i], -1, writer);\n"
                        "                if (i + 1 < count) bugle_io_puts(\", \", writer);\n"
                        "            }\n"
                        "            bugle_io_puts(\" }\", writer);\n"
                        "        }\n"
                        "    }\n",
            ...
            { // block to allow "first" to be declared in this scope
                ...
                while (tmp != NULL_TREE)
                {
                    if (TREE_CODE(tmp) == FIELD_DECL)
                    {
                        ...
                        define = get_type_map(child)->define();
                        fprintf(f,
                                "    budgie_dump_any_type(%s, &value->%s, -1, writer);\n",
...
static void write_parameter_overrides(FILE *f)
{
    for (list<Group>::iterator i = groups.begin(); i != groups.end(); i++)
    {
        ...
        for (int j = (i->has_retn) ? -1 : 0; j < (int) i->parameters.size(); j++)
        {
            ...
            {
                fprintf(f,
                        "static bugle_bool _budgie_dump_%s_%s(const generic_function_call *call, int arg, const void *value, int length, bugle_io_writer *writer)\n"
                        "{\n"
                        "    return (%s);\n"
                        "}\n"
                        "\n",
...
static void write_interceptors(FILE *f)
{
    ...
    for (list<Function>::iterator i = functions.begin(); i != functions.end(); i++)
    {
        ...
        string proto = function_type_to_string(TREE_TYPE(i->node), "BUDGIEAPI " + i->name(),
                                               false, "arg");
        fprintf(f,
                "BUGLE_EXPORT_PRE %s BUGLE_EXPORT_POST;\n"
                "%s\n"
                "{\n"
                "    function_call call;\n",
                proto.c_str(), proto.c_str());
        if (i->group->has_retn)
        {
...
void parser_type(override_type mode, const string &type, const string &code)
{
    ...
    case OVERRIDE_DUMP:
        ...
        o.code = search_replace(o.code, "$W", "(writer)");
